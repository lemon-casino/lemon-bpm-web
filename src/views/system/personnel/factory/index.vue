<template>
  <div class="work-time-container">
    <!-- 紧凑标签页导航 -->
    <div class="compact-tabs" v-if="showTabNavigation">
      <div class="tab-nav">
        <div
          class="tab-btn"
          :class="{ active: activeTab === 'types' }"
          @click="activeTab = 'types'"
          v-if="hasPermiTypeManage"
        >
    ...
    <div class="tab-content-wrapper">
      <!-- 工时类型管理 -->
      <div v-show="showTabNavigation ? activeTab === 'types' : true" v-if="hasPermiTypeManage" class="tab-panel">
        ...
        <div class="panel-content">
          <div class="data-table-container">
            ...
            <div v-if="!typeLoading && typeList.length === 0" class="empty-state">
              ...
              <div class="empty-title">暂无类型</div>
              <div class="empty-description">还没有创建任何类型，立即创建第一个吧</div>
      ...
      <!-- 工作时间配置 -->
      <div v-show="showTabNavigation ? activeTab === 'configs' : true" v-if="hasPermiConfigManage" class="tab-panel">
        <div class="compact-header">
          <div class="header-actions">
            <el-button type="primary" @click="openConfigForm()" size="small">
              <Icon icon="ep:plus" /> 新增配置
            </el-button>
            <el-button v-if="hasPermiConfigBatch" type="success" @click="openBatchConfigForm()" size="small">
                      ...
                      <el-table-column label="操作" width="140" fixed="right" align="center">
                        <template #default="{ row }">
                          <div class="flex gap-1 justify-center">
                            <el-button v-if="hasPermiConfigUpdate" link type="primary" size="small" @click="openConfigForm(row)">
                              <Icon icon="ep:edit" class="mr-2px" /> 修改
                            </el-button>
                            <el-button v-if="hasPermiConfigDelete" link type="danger" size="small" @click="handleDeleteConfig(row.id)">
            ...
            <div v-else class="table-content-wrapper">
              <div class="table-wrapper">
                ...
                >
                  ...
                  <el-table-column prop="typeName" label="类型" min-width="120" align="center">
                    <template #default="{ row }">
                      <el-tag type="success" size="small">{{ row.typeName }}</el-tag>
                    </template>
                  </el-table-column>
                  ...
                  <el-table-column label="信息" width="250"  align="center">
                    <template #default="{ row }">
                      <el-tag type="success" size="small">{{ row.atmCard }}</el-tag>
                    </template>
                  </el-table-column>
                  <el-table-column prop="createTime" label="创建时间" min-width="130" :formatter="dateFormatter" />
                  <el-table-column prop="updateTime" label="修改时间" min-width="130" :formatter="dateFormatter" />
                  <el-table-column label="操作" width="140" fixed="right" align="center">
                    <template #default="{ row }">
                      <div class="flex gap-1 justify-center">
                        <el-button v-if="hasPermiConfigUpdate" link type="primary" size="small" @click="openConfigForm(row)">
...
defineOptions({ name: 'PersonnelFactory' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

/** 权限校验 */
const hasPermiTypeManage = computed(() => {
  return checkPermi(['system:personnel:factory:type'])
})
const hasPermiConfigManage = computed(() => {
  return checkPermi(['system:personnel:factory:config'])
})
const hasPermiConfigUpdate = computed(() => {
  return checkPermi(['system:personnel:factory:config:update'])
})
const hasPermiConfigDelete = computed(() => {
  return checkPermi(['system:personnel:factory:config:delete'])
})
const hasPermiConfigBatch = computed(() => {
  return checkPermi(['system:personnel:factory:config:batch'])
})

// 是否显示标签页导航（只有当两个权限都有时才显示）
const showTabNavigation = computed(() => {
  return hasPermiTypeManage.value && hasPermiConfigManage.value
})

// 工时类型相关
const typeLoading = ref(true)
const typeList = ref<FactoryConfigApi.FactoryTypeVO[]>([])

// 工作时间配置相关
const configLoading = ref(true)
const configList = ref<FactoryConfigApi.WorkTimeConfigVO[]>([])
...
// 根据权限设置默认活跃标签页
const activeTab = ref((() => {
  if (hasPermiTypeManage.value) return 'types'
  if (hasPermiConfigManage.value) return 'configs'
  return 'types' // 默认值
})())
...
const getTypeList = async () => {
  ...
  try {
    ...
    const data = await FactoryConfigApi.FactoryTypeList()
...
const getConfigList = async () => {
  ...
  try {
    ...
    const data = await FactoryConfigApi.getFactoryConfigPage(queryParams)
...
const resetQuery = () => {
  ...
  configQueryParams.name = undefined
...
const openTypeForm = (row?: FactoryConfigApi.FactoryTypeVO) => {
  typeFormRef.value.open(row)
}

/** 工作时间配置表单操作 */
const configFormRef = ref()
const openConfigForm = (row?: FactoryConfigApi.WorkTimeConfigVO) => {
...
const handleDeleteType = async (id: number) => {
  try {
    ...
    await FactoryConfigApi.deleteFactoryType(id)
...
const handleDeleteConfig = async (id: number) => {
  try {
    await message.delConfirm()
    await FactoryConfigApi.deleteFactoryConfig(id)
    message.success(t('common.delSuccess'))
    await getConfigList()
  } catch {}
}
...
@media (max-width: 768px) {
  ...
  /* 分组日期头部优化 */
  .grouped-view .flex.items-center.justify-between {
