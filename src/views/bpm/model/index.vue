<template>
  <ContentWrap>
    ...
    <!-- 按照分类，展示其所属的模型列表 -->
    <div class="px-15px">
      <el-scrollbar height="calc(100vh - 200px)" class="custom-scrollbar">
        <div v-for="item in categoryGroup" :key="item.id">
          <ContentWrap
            class="rounded-lg transition-all duration-300 ease-in-out hover:shadow-xl mb-15px"
            v-loading="loading"
          ...
          >
            <CategoryDraggableModel
              ...
              :categoryInfo="item"
              :autoExpand="!!queryParams.name && item.modelList.length > 0"
              @success="getList"
            />
          </ContentWrap>
        </div>
  ...
  <!-- 弹窗：表单详情 -->
  <Dialog title="表单详情" v-model="formDetailVisible" width="800" :close-on-click-modal="true">
  ...
  try {
    ...
    const processCategories = () => {
      ...
      modelList.forEach((model: any) => {
        if (!modelMap.has(model.categoryName)) {
          modelMap.set(model.categoryName, [])
        }
        modelMap.get(model.categoryName).push(model)
      })

      // 处理分类数据
      categoryGroup.value = categoryList.map((category: any) => ({
        ...category,
        modelList: modelMap.get(category.name) || []
      }))
    }
