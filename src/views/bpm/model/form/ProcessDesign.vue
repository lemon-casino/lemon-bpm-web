...
const formFields = computed(() => {
  // 如果有最新的表单数据，优先使用
  if (props.latestFormData && modelData.value.formId === props.latestFormData.id) {
    console.log('流程设计使用最新的表单字段数据')
    return props.latestFormData.fields || []
  }
  
  // 否则使用模型中的表单字段
  return modelData.value.formFields || []
})
