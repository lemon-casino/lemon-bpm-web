...
<template>
  <el-timeline class="pt-20px">
    <!-- 遍历每个审批节点 -->
    <el-timeline-item
      v-for="(activity, index) in filteredActivityNodes"
      :key="index"
      size="large"
...
import externalLink from '@/assets/svgs/bpm/externalLink.svg'
...
// 过滤重复的结束节点（名称为"结束"且endTime和reason相同的节点只显示一个）
const filteredActivityNodes = computed(() => {
  if (!props.activityNodes || props.activityNodes.length === 0) return []
  
  const uniqueEndNodes = new Map()
  
  return props.activityNodes.filter(node => {
    // 如果不是名为"结束"的节点，直接保留
    if (node.name !== '结束') return true
    
    // 获取节点的唯一标识（结束时间 + 原因）
    const taskReason = node.tasks && node.tasks.length > 0 ? node.tasks[0].reason : ''
    const uniqueKey = `${node.endTime}-${taskReason}`
    
    // 如果这个唯一标识之前没有出现过，则保留该节点
    if (!uniqueEndNodes.has(uniqueKey)) {
      uniqueEndNodes.set(uniqueKey, true)
      return true
    }
...
const statusIconMap2 = {
  ...
  '10': { color: '#00b32a', icon: 'ep:circle-check-filled' }
}
...
const nodeTypeSvgMap = {
  ...
  [NodeType.HTTP_TASK_NODE]: { color: '#14bb83', svg: externalLink }
}
