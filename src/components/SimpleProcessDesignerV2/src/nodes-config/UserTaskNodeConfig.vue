...
<!--        <div class="divide-line"></div>-->
          ...
          <el-form ref="formRef" :model="configForm" label-position="top" :rules="formRules">
            ...
            <div v-if="currentNode.type === NodeType.USER_TASK_NODE">
              ...
              >
                ...
                >
                  <el-radio-button
                    v-for="item in TIMEOUT_HANDLER_TYPES.filter(type => type.value !== TimeoutHandlerType.AUTO_JUMP)"
                    :key="item.value"
                    :value="item.value"
                    :label="item.label"
                  />
              ...
              <el-form-item
                label="最大提醒次数"
                prop="maxRemindCount"
                v-if="configForm.timeoutHandlerEnable && configForm.timeoutHandlerType === TimeoutHandlerType.REMINDER"
              >
              ...
              <el-form-item
                ...
                v-if="configForm.timeoutHandlerEnable && configForm.timeoutHandlerType === TimeoutHandlerType.REMINDER"
              >
                <el-switch v-model="autoJumpEnable" @change="autoJumpEnableChange" />
              </el-form-item>
              <el-form-item
                v-if="configForm.timeoutHandlerEnable && configForm.timeoutHandlerType === TimeoutHandlerType.REMINDER && autoJumpEnable"
                label="跳转节点"
            ...
            <div v-if="currentNode.type === NodeType.USER_TASK_NODE">
              <el-divider content-position="left">工时设置</el-divider>
              <el-form-item label="启用工时计算" prop="workTimeEnable">
                <el-switch
                  v-model="configForm.workTimeEnable"
                  active-text="开启"
                  inactive-text="关闭"
                  @change="workTimeEnableChange"
                />
              </el-form-item>
              <el-form-item
                label="工时类型"
                v-if="configForm.workTimeEnable"
                prop="workTimeType"
              >
                <el-select
                  filterable
                  v-model="configForm.workTimeType"
                  clearable
                  style="width: 100%"
                  :loading="isLoadingWorkTimeTypes"
                  :disabled="isLoadingWorkTimeTypes"
                  placeholder="请选择工时类型"
                >
                  <template #empty>
                    <div v-if="isLoadingWorkTimeTypes" class="loading-container">
                      <el-icon class="is-loading"><Loading /></el-icon>
                      <span class="ml-2">加载中，请稍候...</span>
                    </div>
                    <div v-else-if="workTimeTypeOptions.length === 0" class="empty-data">
                      ...
                      <span class="ml-2">
                        ...
                        <el-button 
                          ...
                          size="small" 
                          class="ml-2" 
                          @click="clearAndRefetchWorkTimeTypes"
                        >
                          重试
                        </el-button>
                      </span>
              ...
              <el-form-item label="工时时间设置" v-if="configForm.workTimeEnable">
                <span class="mr-2">当超过</span>
                <el-form-item prop="workTimeDuration">
                  <el-input-number
                    class="mr-2"
                    :style="{ width: '100px' }"
                    v-model="configForm.workTimeDuration"
                    :min="1"
                    controls-position="right"
                  />
                </el-form-item>
                <el-select
                  filterable
                  v-model="workTimeUnit"
                  class="mr-2"
                  :style="{ width: '100px' }"
                  @change="workTimeUnitChange"
                >
                  <el-option
                    v-for="item in TIME_UNIT_TYPES"
                    :key="item.value"
    ...
    <template #footer>
      ...
      <div>
        ...
        <el-button @click="closeDrawerWithReset">取 消</el-button>
      </div>
    </template>
  </el-drawer>
</template>
...
const formRules = reactive({
  ...
  timeoutReturnNodeId: [{ required: true, message: '跳转节点不能为空', trigger: 'change' }],
  workTimeEnable: [{ required: true }],
  workTimeType: [{ required: true, message: '工时类型不能为空', trigger: 'change' }],
  workTimeDuration: [{ required: true, message: '工时时间不能为空', trigger: 'blur' }]
})
...
const {
  ...
  autoJumpEnable,
  autoJumpEnableChange
} = useTimeoutHandler()

// 工时类型选项
const workTimeTypeOptions = inject<Ref<any[]>>('workTimeTypeOptions', ref([]))
// 工时类型数据加载状态
const isLoadingWorkTimeTypes = inject<Ref<boolean>>('isLoadingWorkTimeTypes', ref(false))
// 获取工时类型数据的方法
const fetchWorkTimeTypes = inject<(forceRefresh?: boolean) => Promise<any[]>>('fetchWorkTimeTypes', async () => [])
// 清除缓存并重新获取工时类型数据的方法
const clearAndRefetchWorkTimeTypes = inject<() => Promise<any[]>>('clearAndRefetchWorkTimeTypes', async () => [])

// 工时时间单位
const workTimeUnit = ref(TimeUnitType.HOUR)

// 工时时间单位改变
const workTimeUnitChange = () => {
  // 分钟，默认是 60 分钟
  if (workTimeUnit.value === TimeUnitType.MINUTE) {
    configForm.value.workTimeDuration = 60
  }
  // 小时，默认是 6 个小时
  if (workTimeUnit.value === TimeUnitType.HOUR) {
    configForm.value.workTimeDuration = 6
  }
  // 天， 默认 1天
  if (workTimeUnit.value === TimeUnitType.DAY) {
    configForm.value.workTimeDuration = 1
  }
}

// 工时时间的 ISO 表示
const workTimeIsoTimeDuration = computed(() => {
  if (!configForm.value.workTimeEnable) {
    return undefined
  }
  let strTimeDuration = ''
  if (workTimeUnit.value === TimeUnitType.MINUTE) {
    strTimeDuration = 'PT' + configForm.value.workTimeDuration + 'M'
  }
  if (workTimeUnit.value === TimeUnitType.HOUR) {
    strTimeDuration = 'PT' + configForm.value.workTimeDuration + 'H'
  }
  if (workTimeUnit.value === TimeUnitType.DAY) {
    strTimeDuration = 'P' + configForm.value.workTimeDuration + 'D'
  }
  return strTimeDuration
})

// 工时计算开关改变
const workTimeEnableChange = () => {
  if (configForm.value.workTimeEnable) {
    // 设置默认值
    workTimeUnit.value = TimeUnitType.HOUR
    configForm.value.workTimeDuration = 6
    
    // 调用注入的fetchWorkTimeTypes方法，不再需要本地实现
    fetchWorkTimeTypes().then(options => {
      // 防御性代码：确保options是数组
      const safeOptions = Array.isArray(options) ? options : [];
      // 如果已启用工时计算但未选择类型，自动选择第一个
      if (configForm.value.workTimeEnable && !configForm.value.workTimeType && safeOptions.length > 0) {
        configForm.value.workTimeType = safeOptions[0].id
      }
    }).catch(error => {
      console.error('获取工时类型数据异常', error)
      ElMessage.warning('获取工时类型数据失败，请稍后重试')
    })
  } else {
    configForm.value.workTimeType = undefined
    configForm.value.workTimeDuration = undefined
  }
}
...
const saveConfig = async () => {
  ...
  currentNode.value.timeoutHandler = {
    ...
    returnNodeId: configForm.value.timeoutHandlerType === TimeoutHandlerType.REMINDER && autoJumpEnable.value ?
      configForm.value.timeoutReturnNodeId : undefined
  }
  // 设置工时计算
  currentNode.value.workTimeHandler = {
    workTimeEnable: configForm.value.workTimeEnable ?? false,
    workTimeType: configForm.value.workTimeEnable ? configForm.value.workTimeType : undefined,
    timeDuration: workTimeIsoTimeDuration.value
  }
  // 设置审批人为空时
...
const showUserTaskNodeConfig = (node: SimpleFlowNode) => {
  ...
  // 设置工时计算
  configForm.value.workTimeEnable = node.workTimeHandler?.workTimeEnable ?? false
  configForm.value.workTimeType = node.workTimeHandler?.workTimeType
  
  // 如果启用了工时计算，设置工时时间
  if (node.workTimeHandler?.workTimeEnable && node.workTimeHandler?.timeDuration) {
    const strTimeDuration = node.workTimeHandler.timeDuration
    
    // 解析工时时间
    let parseTime, parseTimeUnit;
    // 时/分格式：PT1H 或 PT30M
    if (strTimeDuration.startsWith('PT')) {
      parseTime = strTimeDuration.slice(2, strTimeDuration.length - 1)
      parseTimeUnit = strTimeDuration.slice(strTimeDuration.length - 1)
    }
    // 日期格式：P1D
    else if (strTimeDuration.startsWith('P') && !strTimeDuration.includes('T')) {
      parseTime = strTimeDuration.slice(1, strTimeDuration.length - 1)
      parseTimeUnit = strTimeDuration.slice(strTimeDuration.length - 1)
    }
    // 如果将来需要，可以处理 P1DT6H 等组合格式
    else if (strTimeDuration.includes('T')) {
      // For now, just take the first component (days)
      const datePart = strTimeDuration.split('T')[0]
      if (datePart.includes('D')) {
        parseTime = datePart.slice(1, datePart.length - 1)
        parseTimeUnit = 'D'
      }
    }
    
    configForm.value.workTimeDuration = parseInt(parseTime)
    workTimeUnit.value = convertTimeUnit(parseTimeUnit)
  } else {
    // 设置默认值
    configForm.value.workTimeDuration = 6
    workTimeUnit.value = TimeUnitType.HOUR
  ...
  if (node.timeoutHandler?.enable) {
    ...
    if (node.timeoutHandler.type === TimeoutHandlerType.AUTO_JUMP) {
      ...
      // 设置最大提醒次数
      if (node.timeoutHandler.maxRemindCount !== undefined) {
        configForm.value.maxRemindCount = node.timeoutHandler.maxRemindCount
      }
    } else {
      configForm.value.timeoutHandlerType = node.timeoutHandler.type
      
      // 如果是 REMINDER 类型且有 returnNodeId，则启用 autoJumpEnable
      if (node.timeoutHandler.type === TimeoutHandlerType.REMINDER && node.timeoutHandler.returnNodeId) {
        autoJumpEnable.value = true
        configForm.value.timeoutReturnNodeId = node.timeoutHandler.returnNodeId
...
defineExpose({ openDrawer, showUserTaskNodeConfig, closeDrawerWithReset }) // 暴露方法给父组件
...
function useTimeoutHandler() {
  ...
  // 是否启用自动跳转
  const autoJumpEnable = ref(false)
  ...
  const timeoutHandlerChange = () => {
    if (configForm.value.timeoutHandlerEnable) {
      ...
      // 只有在未设置maxRemindCount时才设置默认值
      if (!configForm.value.maxRemindCount) {
  ...
  // 自动跳转开关改变
  const autoJumpEnableChange = () => {
    // 不再在这里设置maxRemindCount，保留用户设置的值
    if (autoJumpEnable.value) {
      // 获取可跳转节点列表
      const matchNodeList: SimpleFlowNode[] = []
      emits('find:returnTaskNodes', matchNodeList)
      // 确保列表不为空
      if (matchNodeList.length > 0) {
        returnTaskList.value = matchNodeList
        console.table(matchNodeList);
        returnTaskList.value.push({
          "id": "EndEvent",
          "name": "结束",
        })
        // 默认选择第一个可用节点
        configForm.value.timeoutReturnNodeId = matchNodeList[0].id
      }
    } else {
      // 关闭自动跳转时，清空跳转节点
      configForm.value.timeoutReturnNodeId = undefined
  ...
  const isoTimeDuration = computed(() => {
    ...
    let strTimeDuration = ''
    if (timeUnit.value === TimeUnitType.MINUTE) {
      strTimeDuration = 'PT' + configForm.value.timeDuration + 'M'
    }
    if (timeUnit.value === TimeUnitType.HOUR) {
      strTimeDuration = 'PT' + configForm.value.timeDuration + 'H'
    }
    if (timeUnit.value === TimeUnitType.DAY) {
      strTimeDuration = 'P' + configForm.value.timeDuration + 'D'
    }
    return strTimeDuration
  })
...
// 组件挂载时预加载工时类型数据的逻辑现在已经移动到ProcessNodeTree.vue中
onMounted(() => {
  // 不再需要在这里预加载工时类型数据
})
</script>
