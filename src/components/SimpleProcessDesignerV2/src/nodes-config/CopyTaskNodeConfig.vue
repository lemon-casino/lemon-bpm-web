        ...
        <div>
          <el-form ref="formRef" :model="configForm" label-position="top" :rules="formRules">
            ...
            <el-form-item
              ...
              span="24"
            >
              <el-select v-model="configForm.userIds" clearable  filterable  multiple style="width: 100%">
                <el-option
            ...
            <el-form-item label="是否抄送本人" prop="copySelf">
              <el-switch v-model="configForm.copySelf" :active-value="true" :inactive-value="false" />
            </el-form-item>
          </el-form>
        </div>
      </el-tab-pane>
...
// 默认 copySelf 为 false
if (configForm.value.copySelf === undefined) {
  configForm.value.copySelf = false
}
// 保存配置
const saveConfig = async () => {
  ...
  // 构建candidateParam (处理 copySelf 为 true 的情况)
  let candidateParam = handleCandidateParam()
  let candidateStrategy = configForm.value.candidateStrategy
  
  // 如果需要抄送本人，且不是"发起人自己"策略，则添加标记
  if (configForm.value.copySelf === true) {
    // 在showText中添加"抄送本人"的提示
    currentNode.value.showText = showText + " - (抄送发起人)"
  } else {
    currentNode.value.showText = showText
  }
  
  currentNode.value.name = nodeName.value!
  currentNode.value.candidateParam = candidateParam
  currentNode.value.candidateStrategy = candidateStrategy
  currentNode.value.copySelf = configForm.value.copySelf // 保存 copySelf
...
const showCopyTaskNodeConfig = (node: SimpleFlowNode) => {
  ...
  // 设置是否抄送本人
  configForm.value.copySelf = node.copySelf ?? false
  // 表单字段权限
