    ...
    >
      ...
      >
        <div class="handler-item-wrapper">
          ...
          <div class="handler-item" @click="addNode(NodeType.HTTP_TASK_NODE)">
            <div class="handler-item-icon http-task">
              <div class="custom-icon">
                <Icon icon="ep:connection" :size="30" />
              </div>
            </div>
            <div class="handler-item-text">HTTP任务</div>
          </div>
        </div>
        <template #reference>
...
const handleDrop = (e) => {
  ...
  try {
    ...
    if (data.node) {
      ...
      const newNode = JSON.parse(JSON.stringify(data.node))
      // Update the node ID to avoid duplicates
      newNode.id = `${newNode.type}_${Date.now()}`
      
      // 保存当前位置的后续节点
      const existingChildNode = props.childNode
      
      // 将新节点插入到当前位置
      newNode.childNode = existingChildNode
      
      // Update the child node reference
      emits('update:childNode', newNode)
    }
  } catch (err) {
...
const addNode = async (type: number) => {
  ...
  if (type === NodeType.HTTP_TASK_NODE) {
    const data: SimpleFlowNode = {
      id: 'Activity_' + generateUUID(),
      name: NODE_DEFAULT_NAME.get(NodeType.HTTP_TASK_NODE) as string,
      showText: '',
      type: NodeType.HTTP_TASK_NODE,
      childNode: props.childNode
    }
    emits('update:childNode', data)
  }
