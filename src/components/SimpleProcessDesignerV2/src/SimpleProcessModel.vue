...
const startTouchDrag = (e: TouchEvent) => {
  ...
  if (e.touches.length !== 1) return // 只支持单指拖拽
  ...
  document.body.setAttribute('data-dragging-container', 'true')
}
...
const stopTouchDrag = (e: TouchEvent) => {
  ...
  document.body.removeAttribute('data-dragging-container')
}
...
.simple-process-model-container {
  ...
  // 移动端触摸优化
  touch-action: none; // 禁用默认的触摸行为，允许自定义拖拽
