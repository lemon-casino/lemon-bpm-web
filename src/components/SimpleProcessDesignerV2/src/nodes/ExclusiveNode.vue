...
const addCondition = () => {
  ...
  if (conditionNodes) {
    ...
    watchConditionNode(conditionData)
  }
}
...
// Inject form configuration and user list
const formConfig = inject<Ref<FormVO>>('formConfig')
const userList = ref<UserVO[]>([])
const loadingUsers = ref(false)
const userMap = inject<Map<string, string>>('userMap', new Map())

// 修改 formatConditionText 函数
const formatConditionTextCache = new Map<string, string>()

const formatConditionText = (text: string) => {
  // 检查缓存
  if (formatConditionTextCache.has(text)) {
    return formatConditionTextCache.get(text)!
  }

  // console.log('formatConditionText input:', text)
  if (!text) return text

  let formattedText = text

  // 处理发起人字段
  formattedText = formattedText.replace(/发起人\s*(等于|不等于|包含|不包含)\s*(\d+)/g, (match, operator, userId) => {
    const userName = userMap.get(userId.toString())
    console.log('处理发起人字段:', { match, operator, userId, userName })
    return `发起人 ${operator} ${userName || userId}`
  })

  // 处理表单字段
  if (formConfig?.value?.fields) {
    const fields = formConfig.value.fields.map(field => {
      try {
        return typeof field === 'string' ? JSON.parse(field) : field
      } catch (e) {
        console.error('字段解析失败:', field, e)
        return field
      }
    })

    fields.forEach(field => {
      if (field.type === 'UserSelect' || field.field === ProcessVariableEnum.START_USER_ID) {
        const escapedTitle = field.title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
        const pattern = new RegExp(`${escapedTitle}\\s*(等于|不等于|包含|不包含)\\s*(\\d+)`, 'g')
        formattedText = formattedText.replace(pattern, (match, operator, userId) => {
          const userName = userMap.get(userId.toString())
          return `${field.title} ${operator} ${userName || userId}`
        })
      }
      
      if (field.type === 'checkbox' || field.type === 'radio') {
        const escapedTitle = field.title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
        const pattern = new RegExp(`${escapedTitle}\\s*(等于|不等于|包含|不包含)\\s*([^\\s]+)`, 'g')
        formattedText = formattedText.replace(pattern, (match, operator, value) => {
          const option = field.options?.find(opt => opt.value.toString() === value.toString())
          return `${field.title} ${operator} ${option?.label || value}`
        })
      }
    })
  }
  ...
  // 存入缓存
  formatConditionTextCache.set(text, formattedText)
  
  // console.log('formatConditionText output:', formattedText)
  return formattedText
}

// 修改 loadUserList 函数，添加更多日志
const loadUserList = async () => {
  if (loadingUsers.value || userList.value.length > 0) {
    console.log('用户列表已加载，跳过:', { 
      loadingUsers: loadingUsers.value, 
      userListSize: userList.value.length,
...
/*    console.log('用户列表加载完成:', {
      ...
      userMapSize: userMap.size,
      userMapEntries: Array.from(userMap.entries()).slice(0, 5) // 只显示前5个用于调试
    })*/
  } catch (error) {
...
// 监听条件变化
watch(() => currentNode.value?.conditionNodes, (newNodes) => {
  if (newNodes && userList.value.length === 0) {
    loadUserList()
  }
  if (newNodes) {
    const debouncedFormat = _.debounce((nodes: SimpleFlowNode[]) => {
      nodes.forEach(node => {
        if (node.showText) {
          node.showText = formatConditionText(node.showText)
        }
      })
    }, 300)
    
    debouncedFormat(newNodes)
  }
}, { deep: true })

// Add watch for individual condition node changes
const watchConditionNode = (node: SimpleFlowNode) => {
  // 使用 lodash 的防抖函数
  const debouncedFormat = _.debounce((text: string) => {
    if (text) {
      node.showText = formatConditionText(text)
    }
  }, 300)

  watch(() => node.showText, (newText) => {
    if (newText) {
      debouncedFormat(newText)
    }
  }, { immediate: true })
}

// Watch for condition nodes array changes to setup watchers for new nodes
watch(() => currentNode.value?.conditionNodes, (newNodes) => {
  if (newNodes) {
    newNodes.forEach(node => {
      watchConditionNode(node)
    })
  }
}, { immediate: true })

// 当用户列表或表单配置更新时，清除缓存
watch([() => userList.value, () => formConfig?.value], () => {
  formatConditionTextCache.clear()
})
</script>
