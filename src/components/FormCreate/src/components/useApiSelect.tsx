...
import { Fragment, defineComponent, onMounted, onUnmounted, ref, useAttrs, watch } from 'vue'
...
type SharedOptionsRecord = {
  expires: number
  options: Record<string, any>[]
}

const SHARED_CACHE_TTL = 5 * 60 * 1000
const sharedOptionsCache = new Map<string, SharedOptionsRecord>()
const sharedPendingRequests = new Map<string, Promise<void>>()

const cloneOptions = (items: Record<string, any>[]) =>
  items.map((item) => ({ ...item }))
...
export const useApiSelect = (option: ApiSelectProps) => {
  return defineComponent({
    ...
    setup(props) {
      ...
      const normalizedMethod = () => (props.method || 'GET').toUpperCase()
      const shouldShareCache = () => !props.remote && !isEmpty(props.url)

      const buildSharedCacheKey = () => {
        const method = normalizedMethod()
        const url = props.url || ''
        const dataKey = method === 'POST' ? `::${props.data || ''}` : ''
        const queryKey = props.remote
          ? `::${props.remoteField}:${queryParam.value ?? ''}`
          : '::static'
        return `${method}::${url}${dataKey}${queryKey}`
      }

      const tryUseSharedOptions = (cacheKey: string) => {
        const record = sharedOptionsCache.get(cacheKey)
        if (!record) {
          return false
        }
        if (record.expires < Date.now()) {
          sharedOptionsCache.delete(cacheKey)
          return false
        }
      ...
      const getOptions = async () => {
        ...
        const canShare = shouldShareCache()
        const cacheKey = buildSharedCacheKey()

        if (canShare) {
          if (tryUseSharedOptions(cacheKey)) {
            return
          }
          const pending = sharedPendingRequests.get(cacheKey)
          if (pending) {
            await pending
            if (tryUseSharedOptions(cacheKey)) {
              return
        ...
        const runRequest = async () => {
          ...
          try {
            ...
            if (canShare) {
              sharedOptionsCache.set(cacheKey, {
                expires: Date.now() + SHARED_CACHE_TTL,
        ...
        if (canShare) {
          const pending = runRequest().finally(() => {
            sharedPendingRequests.delete(cacheKey)
          })
          sharedPendingRequests.set(cacheKey, pending)
          await pending
        } else {
          await runRequest()
        }
