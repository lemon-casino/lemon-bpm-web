...
function normalizePostPayload(
  ...
  if (remote && query !== undefined && query !== null) {
    payload[remoteField] = query
  }
  return {
    payload,
    cacheString: stableStringify(payload)
  }
}
  ...
  return defineComponent({
    ...
    setup(props) {
      ...
      const getOptions = async () => {
        ...
        const currentQuery = props.remote ? queryParam.value ?? '' : ''
        ...
        if (method === 'POST') {
          ...
          cacheString = normalized.cacheString
        }
        ...
        const cacheKey = buildCacheKey({
          ...
          query: props.remote ? currentQuery : '',
          labelField: props.labelField,
          valueField: props.valueField,
          parseFunc: props.parseFunc
        })

        const applyResponse = (responseData: any, fromCache: boolean) => {
          parseOptions(responseData)
          if (!fromCache && options.value && options.value.length > 0) {
        ...
        let pending = pendingResponseCache.get(cacheKey)
        ...
        if (!pending) {
          ...
          pending = (async () => {
            try {
              ...
              if (method === 'GET') {
                ...
                if (props.remote && currentQuery !== undefined && currentQuery !== null) {
                  const encodedQuery = encodeURIComponent(currentQuery)
                  if (url.includes('?')) {
                    url = `${url}&${props.remoteField}=${encodedQuery}`
                  } else {
                    url = `${url}?${props.remoteField}=${encodedQuery}`
                  }
                }
          ...
          pendingResponseCache.set(cacheKey, pending)
        }
        ...
        try {
          ...
          applyResponse(responseData, !createdPending)
          if (createdPending) {
